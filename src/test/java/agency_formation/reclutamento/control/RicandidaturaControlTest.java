...
public class RicandidaturaControlTest {
    ...
    @Test //mi aspetto 3
    public void ricandidaturaNull() throws IOException, ServletException {
    ...
    public void eliminazioneCandidaturaFail() throws IOException, ServletException {
        ...
        Mockito.when(context.getRequestDispatcher(anyString())).thenReturn(dispatcher);
        try (MockedStatic mockedStatic = mockStatic(RicandidaturaControl.class)) {
            mockedStatic.when(() -> RicandidaturaControl.eliminaCandidaturaFromManager(2)).thenReturn(true);
            StringWriter stringWriter = new StringWriter();
            PrintWriter writer = new PrintWriter(stringWriter);
            Mockito.when(response.getWriter()).thenReturn(writer);
            servlet.init(config);
